...
function updateManifestation(manifestation, manifestationId, panelName, buttonValue) {
	...
	let labelName = $('<input type="text" name="updateName' + manifestationId + '" id="nameManifestationUpdate" required>').val(manifestation.name);
	let label2 = $('<label for="typeManifestationUpdate">Manifestation type:</label>');
	let labelTypeManifestation = $('<select name="updateType' + manifestationId + '" id="typeManifestationUpdate"><option value="CONCERT">Concert</option><option value="FESTIVAL">Festival</option><option value="THEATRE">Theatre</option></select>').val(manifestation.typeManifestation);
	let label3 = $('<label for="seatingNumberUpdate">Seating number:</label>');
	let labelSeatingNumber = $('<input type="number" name="updateSeating' + manifestationId + '" id="seatingNumberUpdate" required>').val(manifestation.seatingNumber);
	...
	let labelPriceRegular = $('<input type="number" name="updatePrice' + manifestationId + '" id="priceRegularUpdate" required>').val(manifestation.priceRegular);
...
// ________________________________________________________**********************SELLER ROLE**********************________________________________________________________
function sellerAuthorizations() {
	...
	$.get({
		...
		success: function (manifestations) {
			...
			$('input[name=Change]').click(function () {
				...
				let data = {
					"name": $("input[name=updateName" + manifestationId + "]").val(),
					"typeManifestation": $("select[name=updateType" + manifestationId + "]").val(),
					"seatingNumber": $("input[name=updateSeating" + manifestationId + "]").val(),
					"priceRegular": $("input[name=updatePrice" + manifestationId + "]").val()
				};
	...
	$.get({
		...
		success: function (comments) {
			...
			$('input[name=Deny]').click(function () {
				...
				$.ajax({
					url: 'rest/comments/deny/' + commentId,
					type: 'PUT',
					success: function () {
					...
					error: function () {
						$('#errorActivate').text("Error denying comment!");
						$("#errorActivate").show().delay(8000).fadeOut();
					}
				});
			});
...
// ____________________________________________________________**************BUYER ROLE*******************_________________________________________________________
		...
		success: function (manifestations) {
			...
			$(function () {
				...
				$('input[name=Reserve]').click(function () {
					...
					let manifestationSeatingNumber = $("#seatingNumber" + manifestationId).val();
									...
									$.ajax({
										...
										success: function () {
											...
											$.post({
												...
												data: JSON.stringify(manifestationId),
												contentType: 'application/json',
...
// ____________________________________________________**********************ADMINISTRATOR ROLE***********************_____________________________________________________
function administratorAuthorizations() {
...
$(document).ready(function () {
	...
	$.get({
		...
		success: function (currentUser) {
			...
			// add my profile information to My Profile panel
			...
			$.get({
				...
				success: function (currentUserRole) {
					// BUYER ROLE_________________________________________________________
					if (currentUserRole == "BUYER") {
						buyerAuthorizations();
					}

					// SELLER ROLE________________________________________________________
					else if (currentUserRole == "SELLER") {
						sellerAuthorizations();
					}

					//ADMINISTRATOR ROLE__________________________________________________
					else {
	...
	// _____________________LOGOUT REQUEST_______________________________________________
	...
	//__________________________REQUEST TO SEARCH MANIFESTATIONS_________________________
